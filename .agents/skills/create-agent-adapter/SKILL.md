...
## 2. Shared Types (`@sovereign-clip/adapter-utils`)

All adapter interfaces live in `packages/adapter-utils/src/types.ts`. Import from `@sovereign-clip/adapter-utils` (types) or `@sovereign-clip/adapter-utils/server-utils` (runtime helpers).
...
```json
{
  "name": "@sovereign-clip/adapter-<name>",
  "version": "0.0.1",
  "private": true,
  "type": "module",
  "exports": {
  ...
  "dependencies": {
    "@sovereign-clip/adapter-utils": "workspace:*",
    "picocolors": "^1.1.1"
  },
  "devDependencies": {
    "typescript": "^5.7.3"
...
1. **Read config** — extract typed values from `ctx.config` using helpers (`asString`, `asNumber`, `asBoolean`, `asStringArray`, `parseObject` from `@sovereign-clip/adapter-utils/server-utils`)
...
```ts
import { execute as myExecute, sessionCodec as mySessionCodec } from "@sovereign-clip/adapter-my-agent/server";
import { agentConfigurationDoc as myDoc, models as myModels } from "@sovereign-clip/adapter-my-agent";
...
import { parseMyAgentStdoutLine } from "@sovereign-clip/adapter-my-agent/ui";
import { MyAgentConfigFields } from "./config-fields";
import { buildMyAgentConfig } from "@sovereign-clip/adapter-my-agent/ui";
...
```ts
import { printMyAgentStreamEvent } from "@sovereign-clip/adapter-my-agent/cli";
...
Import from `@sovereign-clip/adapter-utils/server-utils`:
...
- Package name: `@sovereign-clip/adapter-<kebab-name>`
