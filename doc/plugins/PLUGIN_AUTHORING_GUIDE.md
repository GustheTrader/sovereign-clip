...
```bash
pnpm --filter @sovereign-clip/create-paperclip-plugin build
...
Inside this monorepo, the scaffold uses `workspace:*` for `@sovereign-clip/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@sovereign-clip/plugin-sdk` from the local Paperclip checkout into a `.paperclip-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@sovereign-clip/plugin-sdk/ui`
