# `@sovereign-clip/plugin-sdk`

Official TypeScript SDK for Paperclip plugin authors.

- **Worker SDK:** `@sovereign-clip/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@sovereign-clip/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@sovereign-clip/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@sovereign-clip/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@sovereign-clip/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@sovereign-clip/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@sovereign-clip/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, slot prop types |
| `@sovereign-clip/plugin-sdk/ui/hooks` | Hooks only |
| `@sovereign-clip/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@sovereign-clip/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@sovereign-clip/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@sovereign-clip/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@sovereign-clip/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@sovereign-clip/plugin-sdk/types` | Worker context and API types (advanced) |
...
```bash
pnpm add @sovereign-clip/plugin-sdk
```
...
```ts
import { definePlugin, runWorker } from "@sovereign-clip/plugin-sdk";
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@sovereign-clip/plugin-sdk`.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@sovereign-clip/plugin-sdk`.

## UI quick start

```tsx
import { usePluginData, usePluginAction } from "@sovereign-clip/plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@sovereign-clip/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@sovereign-clip/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@sovereign-clip/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@sovereign-clip/plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@sovereign-clip/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@sovereign-clip/plugin-sdk/ui`.
...
```tsx
import type { PluginProjectSidebarItemProps } from "@sovereign-clip/plugin-sdk/ui";
...
import {
  useHostContext,
  usePluginAction,
} from "@sovereign-clip/plugin-sdk/ui";
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@sovereign-clip/plugin-sdk/ui";
...
```ts
import { createTestHarness } from "@sovereign-clip/plugin-sdk/testing";
...
```ts
import { createPluginBundlerPresets } from "@sovereign-clip/plugin-sdk/bundlers";
...
```ts
import { startPluginDevServer } from "@sovereign-clip/plugin-sdk/dev-server";
