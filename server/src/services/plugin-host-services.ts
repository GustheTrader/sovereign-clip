...
import { pluginLogs, agentTaskSessions as agentTaskSessionsTable } from "@sovereign-clip/db";
...
import type {
  ...
  PluginWorkspace,
  IssueComment,
} from "@sovereign-clip/plugin-sdk";
  ...
  return {
    ...
    events: {
      ...
      async subscribe(params: { eventPattern: string; filter?: Record<string, unknown> | null }) {
        const handler = async (event: import("@sovereign-clip/plugin-sdk").PluginEvent) => {
          if (notifyWorker) {
